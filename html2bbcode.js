...
(function (name, definition) {
  ...
  HTMLStack.unescape = function (str, nonbsp) {
    ...
    var escaper = function (match, m1) {
      ...
      if (m) {
        ...
        var code = 0;
        if (match[1] == 'x') {
          code = parseInt(match.substr(2), 16);
        } else {
          code = parseInt(match.substr(1));
        }
        if (code) {
  ...
  HTMLStack.prototype.strip = function (parent) {
    ...
    var new_stack = [];
    var new_len = 0,
      afternewline = parent ? (HTMLTag.newlinetags.indexOf(parent.name) >= 0) : false;
    var stag = true;
    for (var i = 0; i < this.stack.length; i++) {
      var s = this.stack[i];
